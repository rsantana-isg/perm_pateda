"""
LehmerUMDA, LehmerTree — Learning
...
class LearnLehmerUMDA:
    ...
    def __call__(
        ...
        lehmer_codes = self._repr.encode(selected_pop)
        ...
        return {
            ...
            "n_vars": n,
            "model_type": "lehmer_umda",
        }


def learn_lehmer_umda(
    generation: int,
    n_vars: int,
    cardinality: np.ndarray,
    selected_pop: np.ndarray,
...
class LearnLehmerTree:
    ...
    def __call__(
        ...
        lehmer_codes = self._repr.encode(selected_pop)  # shape (m, n)
 
        domain_sizes = np.array([n - i for i in range(n)], dtype=int)
 
        mi_matrix = self._compute_mutual_information(
            lehmer_codes, domain_sizes, laplace_alpha
        )
        ...
        marginal_root = self._learn_marginal(
            lehmer_codes[:, root], domain_sizes[root], laplace_alpha
        )
    ...
    def _compute_mutual_information(
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                # Marginales
                p_i = joint.sum(axis=1)  # shape (di,)
                p_j = joint.sum(axis=0)  # shape (dj,)
 
                # I(X;Y) = Σ_{x,y} P(x,y) log[ P(x,y) / (P(x)P(y)) ]
                outer = np.outer(p_i, p_j)  # shape (di, dj)
                # Máscara para evitar log(0/0)
                mask = (joint > 0) & (outer > 0)
                mi_val = np.sum(
                    joint[mask] * np.log(joint[mask] / outer[mask])
                )
                mi[i, j] = mi_val
                mi[j, i] = mi_val
    ...
    def _maximum_spanning_tree(
        ...
        """
        Construye el árbol generador máximo con el algoritmo de Prim.
 
        Maximiza la suma de información mutua de las aristas del árbol
        (criterio de Chow-Liu).
 
        Returns:
            Lista de aristas (i, j) del árbol, n-1 aristas en total.
        """
        in_tree = [False] * n
        in_tree[0] = True
        edges = []
